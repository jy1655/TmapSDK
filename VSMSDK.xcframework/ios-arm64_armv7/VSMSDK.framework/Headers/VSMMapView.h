...
/** User define의 callout popup 을 출력합니다.
 ...
 @param latitude 위도
 */
- (void)showUserDefineCalloutPopup:(NSString*)popupText
                           popupID:(int)popupID
...
/** 멀티 히트테스트
 ...
 @return 히트 성공 여부
 */
- (bool)multiHitTest:(CGPoint)point tolerance:(CGFloat)tolerance block:(MultiHitTestCompleteBlock)block;
 ...
 * @see VSMHitProperty
 */
- (void)dispatchMultiHitTestResult:(NSArray*)hitObjects
                        hitCallout:(bool)hitCallout
                       showCallout:(bool* _Nullable)showCallout;
